...
queryCtrl.controller('queryCtrl', function($scope, $log, $http, $rootScope, geolocation, gservice){
  ...
  geolocation.getLocation().then(function(data){
    ...
    $scope.formData.logitude = parseFloat(coords.long).toFixed(3);
  ...
  $scope.queryUsers = function(){
    queryBody = {
      longitude:parseFloat($scope.formData.logitude),
    ...
    .success(function(queryResults){
      gservice.refresh(queryBody.latitude, queryBody.logitude, queryResults);
