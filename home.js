jQuery(function($){
  ...
  // Line 11 is an if else statement that says Does a equial b ? the fn returns if the statment is true and the inverse returns if the statment is false
  var ENTER_KEY = 13;
  var ESCAPE_KEY = 27;
  // You are setting up wariables for the enter and escape keys that assign numerical values.
  var ajax = {
    ...
    // This function deletes todo items of particular id from the server.
  ...
  var util = {
    uuid:function(){
      ...
      // The for loop is incrememnting i by 1
      // The random method is multiplying a random number by 16 or 0
      // If i is 8, 12, 16, or 20 it is adding a dash to the id
      // What follows is another if else statment that is added to the id and itself
  ...
  /*
    The pluralize method is using word or words depending on whether count is more than one. The store function is an if else statment that says if the length of arguments is greater than one then it sets the item in local storage as a JSON string. If not, it either parses a string as JSON or returns an array.
  */
  var App ={
    init:function(){
      this.todos = util.store('todos-jquery');
      this.todoTemplate = Handlebars.complie($('#todo-template').html());
      this.footerTemplate = Handlebars.complie($('#footer-template').html());
      this.bindEvents();
      ajax.getJSON(this.intergrateList.bind(this));
    ...
    /*
      ...
      Handlebars is comiling templates as HTML by referencing their element ids
      the bind events function is being called
      the bind method is being called with teh context of `this` for the integrateList function
    ...
    */
    bindEvents:function(){
      ...
      $('#toggle-all').on('change', e => this.edit(e));
    ...
    /*
      This renders the function, bind an event handler to the `focus` javascript event, or trigger that event on an element
    */
    ...
    toggleAll:function(e){
      ...
      /*
        ...
        The toggleAll checks off the todos when they are complte
      */
      this.render();
    },
    ...
    /*
      ...
      The destroy completed function is deleting the task from the server
    */
    ...
    /*
      id represents the closest list items id
    ...
    */
    ...
    /*
      This is recognizing when a todo has changed from false to true and removing it from the array in the server.
    */
    edit:function(e){
    ...
    update:function(e){
      ...
      if($el.data('abort')){
        ...
        var todo = this.todos[this.indexFormEl(el)];
        todo.title = val;
        ajax.update(todo);
      }
      /*
        .trim removes the whitespace from the start and end of strings.
        about and destroy if else statments
      */
      this.render();
    },
    destroy:function(e){
    ...
    integrateList:function(data){
      ...
      .forEach(todo => this.integrate(todo.id, todo.attributes.id, todo.attributes['is-complete']));
      this.render();
    }
  };
  App.init();
